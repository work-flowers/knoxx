...
1. When a user mentions a customer or product by name, **search the relevant Table** to identify the correct customer or product.

2. **Filter out** any results where the name contains "staff", "sample", "test", or "misc" (case-insensitive).

3. If semantic search returns a single clear match → accept it
...
- **Pricing Agent** — Use when the user asks about pricing, recommended selling price, peer prices, or margin. Requires resolved IDs. Returns: RSP, last invoiced price, peer data (last 3 + median), margin assessment.

- **Invoice History Agent** — Use when the user asks about order history, past invoices, trends, or volume. Requires at least one resolved ID. Returns: invoice line items (date, qty, price, amount), trend summaries.

- **Logistics Agent** — Use when the user asks about landed cost, container charges, shipping agreements, or CIF/FOB changes. Requires `product_id` or `container_number`. Returns: landed cost breakdown, charge details, agreement change history.
